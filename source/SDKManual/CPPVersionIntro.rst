...
.. list-table::
   ...
   * - V3.9.7
     ...
     - | 1. Updated robot status feedback structure: added object detection status to gripper motion complete signal; added to gripper faults: 2-command error, 3-workpiece drop, others-gripper fault code +3;
       | 2. Updated code examples for output reset after LUA program stop/pause; optimized load lua file interface to require only the lua file name, path no longer needed.
       | 3. Updated photoelectric sensor TCP calibration code examples; optimized load file interface to require only the lua file name, path no longer needed.
       | 4. Updated code examples for setting speed during trajectory execution; optimized load trajectory J file interface to require only the trajectory J file name, path no longer needed.
       | 5. Updated robot trajectory J file replay code examples; optimized load trajectory J file interface to require only the trajectory J file name, path no longer needed.
       | 6. Updated trajectory replay (trajectory look-ahead) code examples; optimized load trajectory J file interface to require only the trajectory J file name, path no longer needed.
       | 7. LoadDefaultProgConfig() interface job program name parameter now requires only the lua file name, path no longer needed.
       | 8. ProgramLoad() interface job program name parameter now requires only the lua file name, path no longer needed.
       | 9. GetLoadedProgram() interface job program name parameter now requires only the lua file name, path no longer needed.
       | 10. Updated robot LUA program operation code examples; optimized load file interface to require only the lua file name, path no longer needed.
       | 11. Added dexterous hand enable status parameter to SetAxleLuaEnableDeviceType() interface;
       | 12. Added dexterous hand enable status parameter to GetAxleLuaEnableDeviceType() interface;
       | 13. Added dexterous hand enabled device number status parameter to GetAxleLuaEnableDevice() interface;
       | 14. Expanded gripper function code array to 32 in SetAxleLuaGripperFunc() interface, added rotary gripper control, etc.;
       | 15. Expanded gripper function code array to 32 in GetAxleLuaGripperFunc() interface, added rotary gripper status, etc.;
       | 16. Fixed interface name error in SetCoderCompenParams();
       | 17. Added SetDexterousHandsMove() interface for controlling dexterous hand motion.
       | 18. Added SetDexterousHandsAct() interface for controlling dexterous hand reset and activation.
       | 19. Added ClearDexterousHandsError() interface for clearing dexterous hand errors.
       | 20. Added SetDexterousHandsFunc() interface for setting enabled dexterous hand action control functions.
       | 21. Added GetDexterousHandsFunc() interface for getting enabled dexterous hand action control functions.
       | 22. Added SetWeaveBackCenterConfig(), GetWeaveBackCenterConfig() for setting and getting the weave end return to cycle zero point parameters.
       | 23. Added SetWeaveOffsetRT() interface for setting real-time weave offset;
       | 24. Added SetSpeedInstant() interface for real-time speed setting;
